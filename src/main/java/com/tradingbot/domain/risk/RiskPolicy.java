...
/**
 * Чистая бизнес-логика риск-правил.
 * <p>
 * Stateless доменный компонент, не зависящий от инфраструктуры (БД, кэш, транзакции).
 * Выполняет проверку возможности резервирования капитала под ордер.
 */
public class RiskPolicy {

    /**
     * Проверяет возможность резервирования капитала под ордер.
     *
     * @param state текущее состояние риск-системы
     * @param orderId идентификатор ордера
     * @param amount требуемая сумма резерва
     * @return решение риск-менеджмента (approve/reject)
     */
    public static RiskDecision canReserve(RiskState state, UUID orderId, BigDecimal amount) {
        ...
        if (MoneyMath.isLess(state.getBalance(), amount)) {
            ...
            return RiskDecision.reject(
                    RiskDecision.Reason.INSUFFICIENT_CAPITAL,
