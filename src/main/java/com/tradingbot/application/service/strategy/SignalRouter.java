...
public class SignalRouter {
    ...
    public void route(SignalEvent signal) {
        ...
        // ID берём из canonical signal identity.
        SignalEntity entity = SignalEntity.builder()
                ...
                .takeProfit1(
                        signal.getTakeProfit() != null
                                ...
                                : signal.getPrice()
                                  .multiply(java.math.BigDecimal.valueOf(1.0005))
                )
                .takeProfit2(
                        signal.getPrice()
                                .multiply(java.math.BigDecimal.valueOf(1.0010))
                )
                .stopLoss(
                        signal.getStopLoss() != null
                                ? signal.getStopLoss()
                                : signal.getPrice()
                                  .multiply(java.math.BigDecimal.valueOf(0.9995))
                )
        ...
        users.forEach(userEntity -> {
            ...
            if (user.isActive()) {
                String message =
                        signalFormatterService.format(entity, user.getTier());
     ...
     */
    @Deprecated
    private String formatSignalForUser(
            User user,
            SignalEvent signal
    ) {
        return "";
